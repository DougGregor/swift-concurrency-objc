...
final class NWEthernetChannel : CustomDebugStringConvertible {
  enum State : Equatable {
    case setup
    case waiting(NWError)
    case preparing
  ...
  final var stateUpdateHandler: ((NWEthernetChannel.State) -> Void)?
  final let etherType: UInt16
  final let interface: NWInterface
  init(on interface: NWInterface, etherType: UInt16)
  final func start(queue: DispatchQueue)
  final var queue: DispatchQueue? { get }
  final func cancel()
  struct EthernetAddress : Hashable, CustomDebugStringConvertible {
  ...
  final var receiveHandler: ((Data, UInt16, NWEthernetChannel.EthernetAddress, NWEthernetChannel.EthernetAddress) -> Void)?
}
